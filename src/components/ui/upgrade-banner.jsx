...
export function UpgradeBanner({
  buttonText = "Launch Offer",
  description = "₹1999 for first 20 projects | Free Student Portfolios",
  onClose,
  onClick,
  className
}) {
  ...
  return (
    <div className={clsx("mx-auto flex items-center justify-center w-full mt-4", className)}>
      <AnimatePresence>
        <motion.div
          className="relative"
        ...
        >
          ...
          >
            <SettingsFilled className="text-[#006EFE]" />
          </motion.div>
          <motion.div
            initial="hidden"
            animate={isHovered ? "visible" : "hidden"}
          ...
          <div className="relative flex min-h-[35px] w-[95vw] sm:w-auto max-w-2xl items-center justify-between gap-3 rounded-[6px] border border-[#003674] bg-[#06193A] pl-3 pr-2 py-2 sm:py-0 text-sm shadow-md">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-1 sm:gap-2 flex-1">
              <button
                className="cursor-pointer border-none bg-transparent px-0 py-0 sm:py-1 font-sans text-[13px] font-medium text-[#EAF5FF] underline decoration-[#003674] underline-offset-[5px] outline-none hover:text-[#44A7FF] hover:decoration-[#00408A] transition-colors text-left leading-tight"
              ...
              <span className="text-[12px] sm:text-[13px] text-[#44A7FF] leading-tight">
                {description}
              </span>
            </div>
            {onClose && (
              <button 
                onClick={onClose}
                className="flex h-6 w-6 shrink-0 cursor-pointer items-center justify-center rounded-[4px] border-0 bg-transparent p-0 text-[#47A8FF] hover:bg-[#012F61] transition-colors self-start sm:self-center mt-0.5 sm:mt-0"
              >
                <X size={16} />
              </button>
            )}
